...
const Checkout = () => {
  ...
  const { cartItems, subtotal } = (route.params as RouteParams) || { cartItems: [], subtotal: 0 };
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      Alert.alert(
        ...
        `Your order #${orderRef.id.slice(-5).toUpperCase()} has been placed and will be delivered soon.`,
        [
          {
            text: 'View Order',
            onPress: () => navigation.navigate('OrderDetails', { orderId: orderRef.id })
          },
          {
